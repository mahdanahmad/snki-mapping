...
function shpToJson(filepath, callback) {
	...
	shapefile.read(filepath, filepath.slice(0, -3) + 'dbf').then(result => { callback(result); });
...
fs.readdirSync(shp_root).filter(o => (path.extname(o) == '.shp')).forEach(file => {
	...
	shpToJson(file, geo => { toTopo(filename, geo, topo => { writeToFile(rslt_name, topo); }); });
	// shpToJson(file, geo => { console.log(geo.features.map(o => o.properties)); });
});
